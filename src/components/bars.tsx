...
/** Pokémon battle HP bar — value is recovery score 0–100 */
export function HPBar({ value, label = "HP" }: { value: number | null; label?: string }) {
  ...
  return (
    <div className="flex items-center gap-2 w-full">
      ...
      <div className="flex-1 h-3 bg-gb-pale border-2 border-gb-darkest p-px">
        <div
          className="h-full"
...
/** XP bar — progress 0..1 */
export function XPBar({ progress, label = "EXP" }: { progress: number; label?: string }) {
  const pct = Math.max(0, Math.min(100, progress * 100));
  return (
    <div className="flex items-center gap-2 w-full">
      <span className="font-pixel text-[9px]">{label}</span>
      <div className="flex-1 h-2 bg-gb-pale border-2 border-gb-darkest">
        <div className="h-full bg-gb-darkest" style={{ width: `${pct}%` }} />
      </div>
    </div>
  );
}
...
export function ZoneBar({ zoneMs, legend = true }: { zoneMs: number[]; legend?: boolean }) {
  ...
  if (total === 0) return <p className="text-gb-dark">No zone data</p>;
  return (
    <div>
      <div className="flex h-5 w-full border-2 border-gb-darkest">
        {zoneMs.map((ms, i) =>
          ms > 0 ? (
      ...
      {legend && (
        <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5">
          {zoneMs.map((ms, i) =>
            ms > 0 ? (
              <span key={i} className="flex items-center gap-1 text-base text-gb-darkest">
                <span className="inline-block size-2.5 border border-gb-darkest" style={{ background: ZONE_COLORS[i] }} />
